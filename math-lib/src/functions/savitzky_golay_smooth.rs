...
#[wasm_bindgen]
pub fn savitzky_golay_smooth(window_size: usize, poly_order: usize, coords_js: Float64Array) -> Float64Array {
    ...
    for i in 0..n {
        if i < half || i >= n - half {
            // edges: return original
            result.push(coords[2 * i]);
            result.push(coords[2 * i + 1]);
            continue;
        }
        // interior: fit polynomial and take c[0] at relative x=0
        ...
        let smooth_y = coeffs[0];
        result.push(center);
        result.push(smooth_y);
    }
